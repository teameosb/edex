...
import (
	...
	"errors"
	"fmt"
	"github.com/teameosb/eosb-sdk-backend/engine"
	"math/big"
	"os"
	"runtime"
	"time"
...
func (m MarketHandler) handleNewOrder(event *common.NewOrderEvent) (transaction *models.Transaction, launchLog *models.LaunchLog) {
	...
	matchResult, hasMatch := m.eosbEngine.HandleNewOrder(eventMemoryOrder)
	if hasMatch {
...
func processTransactionAndLaunchLog(matchResult *MatchResultWithOrders) (*models.Transaction, *models.LaunchLog) {
	...
	eosbTakerOrder := geteosbOrderFromModelOrder(takerOrder.GetOrderJson())

	var eosbMakerOrders []*sdk.Order
	...
	for _, item := range matchResult.MatchItems {
		...
		eosbMakerOrder := geteosbOrderFromModelOrder(modelMakerOrder.GetOrderJson())
		eosbMakerOrders = append(eosbMakerOrders, eosbMakerOrder)
	...
	launchLog := &models.LaunchLog{
		...
		Data:      utils.Bytes2HexP(eosbProtocol.GetMatchOrderCallData(eosbTakerOrder, eosbMakerOrders, baseTokenFilledAmounts)),
		CreatedAt: time.Now().UTC(),
...
func (m *MarketHandler) handleCancelOrder(event *common.CancelOrderEvent) (interface{}, error) {
	...
	msg, success := m.eosbEngine.HandleCancelOrder(bookOrder)
	if success {
		pushMessage(msg)
	}
...
func NewMarketHandler(ctx context.Context, market *models.Market, engine *engine.Engine) (*MarketHandler, error) {
	...
	// re-insert available orders into eosbEngine
