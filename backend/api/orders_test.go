...
func mockErc20() {
	mockeosb := sdk.Mockeosb{
		&ethereum.EthereumEosbProtocol{},
		&sdk.MockBlockchain{},
	}
	...
	mockeosb.BlockChain.(*sdk.MockBlockchain).On("GetTokenBalance", mock.Anything, mock.Anything).Times(10).Return(aBigNumber, nil)
	mockeosb.BlockChain.(*sdk.MockBlockchain).On("GetTokenAllowance", mock.Anything, mock.Anything, mock.Anything).Times(10).Return(aBigNumber, nil)
	mockeosb.BlockChain.(*sdk.MockBlockchain).On("GetHotFeeDiscount", mock.Anything).Times(10).Return(decimal.New(1, 2))
	mockeosb.BlockChain.(*sdk.MockBlockchain).On("IsValidSignature", mock.Anything, mock.Anything, mock.Anything).Return(true, nil)
}
...
func (suite *BuildOrderTestSuit) request(body interface{}) *Response {
	...
	message := "EOSB-AUTHENTICATION"
	req.Header.Set("Eosb-Authentication", address+"#"+message+"#"+signatrue)
