...
func PlaceOrder(p Param) (interface{}, error) {
	...
	if valid := eosb.IsValidOrderSignature(order.Address, order.ID, order.Signature); !valid {
...
func checkBalanceAllowancePriceAndAmount(order *BuildOrderReq, address string) error {
	...
	baseTokenBalance := eosb.GetTokenBalance(market.BaseTokenAddress, address)
	baseTokenAllowance := eosb.GetTokenAllowance(market.BaseTokenAddress, os.Getenv("HSK_PROXY_ADDRESS"), address)
	...
	quoteTokenBalance := eosb.GetTokenBalance(market.QuoteTokenAddress, address)
	quoteTokenAllowance := eosb.GetTokenAllowance(market.QuoteTokenAddress, os.Getenv("HSK_PROXY_ADDRESS"), address)
...
func BuildAndCacheOrder(address string, order *BuildOrderReq) (*BuildOrderResp, error) {
	...
	orderData := eosb.GenerateOrderData(
		int64(2),
	...
	orderHash := eosb.GetOrderHash(sdkOrder)
