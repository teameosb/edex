...
import (
	...
	"github.com/teameosb/eosb-sdk-backend/launcher"
...
func run() int {
	...
	// blockchain
	eosb := ethereum.NewEthereumeosb(os.Getenv("HSK_BLOCKCHAIN_RPC_URL"), os.Getenv("HSK_HYBRID_EXCHANGE_ADDRESS"))
	if os.Getenv("HSK_LOG_LEVEL") == "DEBUG" {
		eosb.EnableDebug(true)
	}

	signService := launcher.NewDefaultSignService(os.Getenv("HSK_RELAYER_PK"), eosb.GetTransactionCount)

	fallbackGasPrice := decimal.New(3, 9) // 3Gwei
	priceDecider := launcher.NewGasStationGasPriceDecider(fallbackGasPrice)

	launcher := launcher.NewLauncher(ctx, signService, eosb, priceDecider)
