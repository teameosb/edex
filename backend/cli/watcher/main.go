...
import (
	"context"
	"encoding/json"
	"github.com/teameosb/eth-watcher/plugin"
	"github.com/teameosb/eth-watcher/structs"
	"strconv"
	...
	"github.com/teameosb/eth-watcher"
	"os"
)
...
func (handler DBTransactionHandler) TxHandlerFunc(txAndReceipt *structs.RemovableTxAndReceipt) {
	...
	if launchLog.ItemType == "eosbApprove" {
	...
	handler.kvStore.Set(common.EOSB_WATCHER_BLOCK_NUMBER_CACHE_KEY, strconv.FormatUint(tx.GetBlockNumber(), 10), 0)
}
...
func main() {
	...
	queue, err := common.InitQueue(&common.RedisQueueConfig{
		Name:   common.EOSB_ENGINE_EVENTS_QUEUE_KEY,
		Client: client,
		Ctx:    ctx,
	})
	if err != nil {
	...
	syncedBlockInCache, err := kvStore.Get(common.EOSB_WATCHER_BLOCK_NUMBER_CACHE_KEY)
